...
async function seedDefaults() {
  ...
  if (!intruderBotId) {
    ...
    await pool.query(
      "UPDATE users SET rank_name = 'bot', display_name = 'Intruder', avatar_url = '/assets/intruder-bot.png', bio = 'Intruder game bot.', profile_status = 'Invisible', show_online_status = 0 WHERE id = ?",
      [intruderBotId]
    );
  }
  await pool.query(
    "UPDATE users SET rank_name = 'bot', profile_status = 'Invisible', show_online_status = 0 WHERE id = ?",
    [intruderBotId]
  );
