...
router.get("/dashboard", async (req, res) => {
  ...
  const [users] = await pool.query(
    `SELECT id, username, email, rank_name, avatar_url, ip_address, xp, gold, diamonds, muted_until, kicked_until, banned_until, last_seen, created_at
     FROM users
  ...
  res.json({
    ...
    toolAccess: {
      intruderTool: Boolean(permissions.find((p) => p.rank_name === "chief" && p.tool === "intruderTool" && p.allowed)),
      profileEditTool: Boolean(permissions.find((p) => p.rank_name === "chief" && p.tool === "profileEditTool" && p.allowed)),
    },
  });
});
