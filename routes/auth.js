...
router.get("/me", requireAuth, async (req, res) => {
  if (PURCHASABLE_RANKS.has(req.user.rank_name) && req.user.rank_until && new Date(req.user.rank_until) < new Date()) {
    const restoredRank = req.user.rank_base && req.user.rank_base !== "bot" ? req.user.rank_base : "user";
    await pool.query("UPDATE users SET rank_name = ?, svip_until = NULL, rank_until = NULL, rank_plan = NULL, rank_base = NULL WHERE id = ?", [restoredRank, req.user.id]);
    req.user.rank_name = restoredRank;
